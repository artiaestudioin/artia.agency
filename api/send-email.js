...
// sanitize definida UNA sola vez
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;">
  ...
  <tr><td style="background:#ffffff;padding:36px 40px 32px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #e2e8f0;border-radius:10px;overflow:hidden;margin-bottom:28px;border-collapse:separate;">
      ...
      <tr style="background:#fafbfc;">
        ...
        <td style="padding:14px 20px;vertical-align:top;">
          <p style="margin:0;font-size:14px;color:#334155;line-height:1.75;">${cleanMessage || 'El prospecto no dejó mensaje adicional.'}</p>
        </td>
      </tr>
    </table>
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px;">
      <tr>
        <td style="background:#00113a;border-radius:8px;">
          <a href="mailto:${cleanEmailFrom}?subject=Re: Consulta ${folio} — ${cleanName}" style="display:inline-block;padding:14px 32px;color:#fff;font-size:11px;font-weight:900;letter-spacing:2px;text-transform:uppercase;text-decoration:none;">
  ...
  <tr><td style="background:#00113a;border-radius:0 0 12px 12px;padding:20px 40px;text-align:center;">
    <p style="margin:0 0 6px;font-size:10px;color:rgba(179,197,255,0.5);line-height:1.6;">
      ...
      Fue generado automáticamente desde el portal web.
    </p>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;">
  ...
  <tr><td style="background:#ffffff;padding:36px 40px 32px;">
    ...
    <p style="margin:0 0 20px;font-size:15px;color:#334155;line-height:1.7;">
      Gracias por contactar a <strong>ARTIA Studio</strong>. Hemos recibido exitosamente tu solicitud de consultoría a través de nuestro portal web.
    </p>
    ...
    <p style="margin:0 0 28px;font-size:15px;color:#334155;line-height:1.7;">
      Uno de nuestros asesores revisará tu requerimiento y se pondrá en contacto en un plazo máximo de <strong>24 horas hábiles</strong>.
    </p>
  ...
  <tr><td style="background:#00113a;border-radius:0 0 12px 12px;padding:20px 40px;text-align:center;">
    ...
    <p style="margin:0;font-size:9px;color:rgba(255,255,255,0.3);">
      © ${year} ARTIA Studio · Por favor no respondas a este correo generado automáticamente.
    </p>
  </td></tr>
  ...
  try {
    ...
    if (!responseArtia.ok) {
      const errBody = await responseArtia.text();
      console.error('Error enviando correo interno:', errBody);
      throw new Error('Error enviando el correo interno');
    }
    ...
    if (!responseClient.ok) {
      const errBody = await responseClient.text();
      console.error('Error enviando confirmación al cliente:', errBody);
      // No lanzamos error aquí para no bloquear la respuesta si el correo
      // interno ya salió correctamente
    }
